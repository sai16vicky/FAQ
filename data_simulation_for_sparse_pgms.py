...
no_of_factors = no_of_variables + numpy.random.randint(1, 2**no_of_variables - 1)
...
for each_factor in range(0, no_of_factors):
    ...
    if len(cur_factor) > 1 and len(cur_factor) <= max_size_of_factors:
        factor_set.add(frozenset(cur_factor))
...
for factor in factor_set:
    ...
    file_write.write('\n')
    number_of_zeros = int((sparsity_factor/100.0) * factor_size)
